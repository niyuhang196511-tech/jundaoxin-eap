...
def test_lexical_rerank_improves_head_precision(client):
    """构造 RRF 召回序与词面覆盖度分歧的场景：重排后覆盖度高的候选排第一。"""
    kb = client.post("/api/v1/kb", headers=HEADERS,
                     json={"name": "rerank-kb", "title": "rerank-kb", "template": "doc"}).json()
    name = kb["name"]
...
def test_llm_rerank_falls_back_gracefully(client):
    """llm 重排：mock 回声不可解析 → 自动回退 lexical（确定性），不报错不丢结果。"""
    kb = client.post("/api/v1/kb", headers=HEADERS,
                     json={"name": "rerank-llm-kb", "title": "rerank-llm-kb",
                           "template": "doc"}).json()
    name = kb["name"]
    client.post(f"/api/v1/kb/{name}/documents", headers=HEADERS,
                json={"title": "短文", "text": "退货政策 三十天无理由退货。"})
...
def test_no_rerank_default_unchanged(client):
    """默认不重排：不传 rerank 时行为与历史一致（排序 = 纯 RRF 融合序）。"""
    kb = client.post("/api/v1/kb", headers=HEADERS,
                     json={"name": "rerank-off-kb", "title": "rerank-off-kb", "template": "doc"}).json()
    name = kb["name"]
    client.post(f"/api/v1/kb/{name}/documents", headers=HEADERS,
