...
def test_rbac_member_blocked_admin_only(client, fake_idp, uname):
    ...
    model = uname("rbac-test-model")  # M52-D：唯一名——脏库重跑 admin 建档不撞 409
    ...
    resp = client.post("/api/v1/models", headers=AUTH, json={
        "name": uname("rbac-test-model-2"), "capabilities": ["chat"]})
    assert resp.status_code == 200, resp.text
