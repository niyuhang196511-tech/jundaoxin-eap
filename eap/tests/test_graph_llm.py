...
def test_llm_extraction_creates_typed_relations(client, llm_mode, uname):
    kb = uname("graph-llm-kb")  # M52-D：唯一名——每遍运行在全新 KB 上断言，免受残留文档干扰
    client.post("/api/v1/kb", headers=HEADERS,
                json={"name": kb, "title": "graph-llm-kb", "template": "doc"})
    client.post(f"/api/v1/kb/{kb}/documents", headers=HEADERS,
                json={"title": "订单流", "text": "下单后 order 与 inventory 联动更新 stock 数据。"})
    g = client.get(f"/api/v1/kb/{kb}/graph", headers=HEADERS).json()
...
def test_llm_extraction_failure_falls_back_to_cooccurrence(client, llm_mode, uname):
    """纯中文内容 mock 抽取为空 → 自动回退词元共现（图谱不为空）。"""
    kb = uname("graph-fallback-kb")
    client.post("/api/v1/kb", headers=HEADERS,
                json={"name": kb, "title": "graph-fallback-kb", "template": "doc"})
    client.post(f"/api/v1/kb/{kb}/documents", headers=HEADERS,
...
def test_lexical_mode_default_unchanged(client, uname):
    """默认 lexical 模式：行为与既有共现图一致。"""
    kb = uname("graph-lex-kb")
    client.post("/api/v1/kb", headers=HEADERS,
                json={"name": kb, "title": "graph-lex-kb", "template": "doc"})
    client.post(f"/api/v1/kb/{kb}/documents", headers=HEADERS,
