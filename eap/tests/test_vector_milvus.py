...
def test_milvus_vector_path_end_to_end(client, milvus_client):
    ...
    try:
        ...
        r = client.post("/api/v1/kb/milvus-kb/documents", headers=HEADERS,
        ...
        hits = client.post("/api/v1/kb/milvus-kb/retrieve", headers=HEADERS,
                           json={"query": "EAP一体机 配送", "top_k": 3}).json()["hits"]
        assert hits and hits[0]["citation"]["document"] == "milvus-doc"

        # 级联删除 → Milvus 中向量同步清理
        doc_id = client.get("/api/v1/kb/milvus-kb/documents", headers=HEADERS).json()[0]["id"]
