...
# ---------- M52-D 可重入命名 ----------
# 设备名跨运行唯一（样板同 test_connector_v2._SFX）：设备「在用重名 → 409」且本模块
# 不吊销收尾，固定名脏库重跑必撞 EAP-2002；唯一名同时保证审计 actor/target 查询
# （harness:<设备名>:<user>）只命中本遍行——len(rows)==1 这类精确计数不被上一遍残留击穿。
_SFX = uuid.uuid4().hex[:8]

REP1 = f"rep-dev-1-{_SFX}"
REP2 = f"rep-dev-2-{_SFX}"
REP3 = f"rep-dev-3-{_SFX}"
REP4 = f"rep-dev-4-{_SFX}"
REP5 = f"rep-dev-5-{_SFX}"
...
def test_device_report_entries_and_prefix_force(client: TestClient):
    ...
    key = _register(client, REP1, user="zhang.san")["api_key"]
    ...
    rows = _query_audit(client, actor=f"harness:{REP1}:zhang.san")
...
def test_report_batch_limit_422(client: TestClient):
    key = _register(client, REP2)["api_key"]
...
def test_device_user_placeholder_in_actor(client: TestClient):
    ...
    key = _register(client, REP3)["api_key"]
    assert _report(client, key, [{"action": "skill.run", "detail": "x"}]).json() == {"accepted": 1}
    rows = _query_audit(client, actor=f"harness:{REP3}:?")
    assert len(rows) >= 1
    assert all(row["target"] == REP3 for row in rows)


def test_report_self_audit(client: TestClient):
    """上报动作本身落 harness.audit.report（仅条数，不含条目内容）。"""
    key = _register(client, REP4, user="li.si")["api_key"]
    assert _report(client, key, [
        {"action": "skill.run", "detail": "a"},
        {"action": "skill.remove", "detail": "b"},
    ]).json() == {"accepted": 2}
    rows = _query_audit(client, action="harness.audit.report", actor=f"harness:{REP4}:li.si")
    assert len(rows) == 1
...
def test_report_created_at_iso_and_missing(client: TestClient):
    ...
    key = _register(client, REP5)["api_key"]
    r = _report(client, key, [
    ...
    assert r.json() == {"accepted": 3}
    rows = _query_audit(client, actor=f"harness:{REP5}:?")  # 含条目行与上报自审计行
