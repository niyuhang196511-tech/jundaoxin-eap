...
| 知识中心 | 多 KB、分块、hash/openai 嵌入、**三路检索：BM25+向量+图谱（实体共现图，多跳扩展召回，/kb/{name}/graph 概览）+ RRF、Citation、级联删除（含图谱/向量库）、向量路可切 Milvus（EAP_MILVUS_URI，不可达自动回退本地余弦）** | 真实实体抽取（LLM）、Reranker |
