...
> **📌 进度账本**：本文件是 v1.0 功能全景**设计文档**（目标态）。各项功能的**实现状态、剩余工作、验收标准与并行分组**见 [progress-plan.md](./progress-plan.md)——开发前先看那份，完成一项回写一项，无需重新探查代码库。以下为设计原文（快照于 v0.4.0 时期，其中 v0.5~v0.9 各阶段规划已全部落地，平台版本 v0.9.0 已发布）。
