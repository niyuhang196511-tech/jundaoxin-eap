...
| 最新里程碑 | M42（modelhub-M42A vLLM+LoRA / evals-M42B 评测门禁接入路由）——L1 收官 |
| 代码 commit | 736d1bf |
| 快照日期 | 2026-09-21 |
| 测试基线 | **393 passed, 8 skipped, 0 errors**（96s）——较 M41 基线 376 增 17（M42 两线新用例 13+4） |——较 M38 基线 342 增 13（M39-A Rust 13 cargo test 在 harness 仓、平台侧增量=M39-B 9 用例+迁移后 agent 测试归位）；Harness 侧以 cargo test 13 绿 + tauri build 为验收 |
| 下一主线 | ~~组织记忆联动 KB~~ 已落地（M41-A，批次 8-A）；L 组其余按外部条件消化（vLLM 需 GPU、IM 联调需外部账号、SLO 需生产环境、计费需产品决策） |
...
| 批次 8+ | Harness 桌面端 v1.1 主线（形态已决策：订阅+快捷调用 + 数据私有化 + 本地技能）＋ L 组其余外部条件项 | vLLM（GPU）/ IM 联调（外部账号）/ SLO（生产环境）/ 计费（产品决策） |
